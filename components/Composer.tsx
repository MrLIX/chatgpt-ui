...
export function Composer(props: {
  ...
  } = useConversationPartial(props.conversationId);
  ...
  const onSpeechResultCallback = React.useCallback((transcript: string) => {
    setComposeText((current) => {
      ...
      if (
        (!current || current.endsWith(".") || current.endsWith("!") || current.endsWith("?")) &&
        transcript.length
      )
    ...
    for (let clipboardItem of await navigator.clipboard.read()) {
      ...
      try {
        ...
        if (htmlString.indexOf("<table") == 0) {
          ...
          setComposeText(
            expandPromptTemplate(PromptTemplates.PasteMarkdown, { clipboard: markdownString })
          );
          continue;
        }
      ...
      try {
        ...
        setComposeText(
          expandPromptTemplate(PromptTemplates.PasteMarkdown, { clipboard: textString })
        );
        continue;
      } catch (error) {
        // ignore missing text
  ...
  const showHistory = (event: React.MouseEvent<HTMLAnchorElement>) =>
  ...
  const handleOverlayDrop = async (e: React.DragEvent) => {
    ...
    if (droppedText?.length >= 1)
      return setComposeText((text) =>
        expandPromptTemplate(PromptTemplates.PasteMarkdown, { clipboard: droppedText })(text)
      );
  ...
  const textPlaceholder: string = `Type ${
    props.isDeveloperMode ? "your message and drop source files" : "a message, or drop text files"
  }...`;
    ...
    <Grid container spacing={{ xs: 1, md: 2 }}>
      ...
      <Grid xs={12} md={9}>
        <Stack direction="row" spacing={{ xs: 1, md: 2 }}>
          ...
          {/* [desktop-only] Edit box, with Drop overlay */}
          <Box sx={{ flexGrow: 1, position: "relative", marginLeft: "0 !important" }}>
            <Textarea
              variant="outlined"
              autoFocus
