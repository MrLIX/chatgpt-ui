...
export default async function handler(req: NextRequest): Promise<Response> {
  ...
  const apiCommon: ApiCommonInputs = {
    apiKey: (userApiKey || process.env.NEXT_PUBLIC_OPENAI_API_KEY || "").trim(),
    apiHost: (userApiHost || process.env.OPENAI_API_HOST || "api.openai.com")
      .trim()
      .replaceAll("https://", ""),
  ...
  if (!apiCommon.apiKey)
    return new Response(
      "[Issue] missing OpenAI API Key. Add it on the client side (Settings icon) or server side (your deployment).",
      { status: 400 }
    );
